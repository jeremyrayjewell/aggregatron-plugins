...
namespace
{
using ScaleTemplate = AggregaScaleAudioProcessor::ScaleTemplate;
...
DetectionResult detectScaleFromAudio(const juce::AudioBuffer<float>& source, double sampleRate, std::atomic<float>& progress)
{
    ...
    for (int root = 0; root < 12; ++root)
    {
        ...
        {
            auto mask = AggregaScaleAudioProcessor::buildMaskFromTemplate(root, scaleTemplate);
    ...
    const auto matchedMask = AggregaScaleAudioProcessor::buildMaskFromTemplate(bestRoot, *bestTemplate);
    ...
    for (int pitchClass = 0; pitchClass < 12; ++pitchClass)
    {
        ...
        if (energy >= totalEnergy * 0.09)
            prominentNotes.add(AggregaScaleAudioProcessor::pitchClassName(pitchClass));
    }
    ...
    result.valid = true;
    result.detailText = "Detected " + AggregaScaleAudioProcessor::pitchClassName(bestRoot) + " " + bestTemplate->name
...
AggregaScaleAudioProcessor::AggregaScaleAudioProcessor()
...
void AggregaScaleAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    juce::ignoreUnused(sampleRate, samplesPerBlock);
}

void AggregaScaleAudioProcessor::releaseResources()
{
}

#ifndef JucePlugin_PreferredChannelConfigurations
bool AggregaScaleAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
...
void AggregaScaleAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
...
juce::AudioProcessorEditor* AggregaScaleAudioProcessor::createEditor()
{
    return new AggregaScaleAudioProcessorEditor(*this);
}

bool AggregaScaleAudioProcessor::hasEditor() const { return true; }
const juce::String AggregaScaleAudioProcessor::getName() const { return "AggregaScale"; }
bool AggregaScaleAudioProcessor::acceptsMidi() const { return false; }
bool AggregaScaleAudioProcessor::producesMidi() const { return false; }
bool AggregaScaleAudioProcessor::isMidiEffect() const { return false; }
double AggregaScaleAudioProcessor::getTailLengthSeconds() const { return 0.0; }
int AggregaScaleAudioProcessor::getNumPrograms() { return 1; }
int AggregaScaleAudioProcessor::getCurrentProgram() { return 0; }
void AggregaScaleAudioProcessor::setCurrentProgram(int index) { juce::ignoreUnused(index); }
const juce::String AggregaScaleAudioProcessor::getProgramName(int index) { juce::ignoreUnused(index); return {}; }
void AggregaScaleAudioProcessor::changeProgramName(int index, const juce::String& newName) { juce::ignoreUnused(index, newName); }

AggregaScaleAudioProcessor::ScaleState AggregaScaleAudioProcessor::getScaleState() const
{
    const juce::ScopedLock lock(stateLock);
    return scaleState;
}

void AggregaScaleAudioProcessor::setScaleState(const ScaleState& newState)
{
    const juce::ScopedLock lock(stateLock);
    scaleState = newState;
}

std::array<bool, 12> AggregaScaleAudioProcessor::buildMaskFromTemplate(int rootNote, const ScaleTemplate& scaleTemplate)
{
...
const AggregaScaleAudioProcessor::ScaleTemplate* AggregaScaleAudioProcessor::findScaleTemplate(const juce::String& scaleName)
{
    for (const auto& scaleTemplate : scaleTemplates)
...
juce::String AggregaScaleAudioProcessor::pitchClassName(int pitchClass)
{
...
void AggregaScaleAudioProcessor::applyPresetScale(int rootNote, const juce::String& scaleName)
{
...
void AggregaScaleAudioProcessor::applyCustomScale(int rootNote, const std::array<bool, 12>& enabledPitchClasses, const juce::String& label)
{
    ScaleState newState;
    newState.rootNote = juce::negativeAwareModulo(rootNote, 12);
...
void AggregaScaleAudioProcessor::joinLoaderThread()
{
    if (loaderThread.joinable())
        loaderThread.join();
}

bool AggregaScaleAudioProcessor::startLoadingSourceFile(const juce::File& file)
{
...
void AggregaScaleAudioProcessor::performLoad(const juce::File& file)
{
...
void AggregaScaleAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
    auto state = getScaleState();
    juce::ValueTree tree("AggregaScaleState");
    tree.setProperty("rootNote", state.rootNote, nullptr);
    tree.setProperty("scaleName", state.scaleName, nullptr);
    tree.setProperty("sourceLabel", state.sourceLabel, nullptr);
    tree.setProperty("detailText", state.detailText, nullptr);
    tree.setProperty("derivedFromAnalysis", state.derivedFromAnalysis, nullptr);

    juce::String maskBits;
    for (const auto enabled : state.enabledPitchClasses)
        maskBits << (enabled ? "1" : "0");
    tree.setProperty("maskBits", maskBits, nullptr);

    if (auto xml = tree.createXml())
        copyXmlToBinary(*xml, destData);
}

void AggregaScaleAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
    if (auto xml = getXmlFromBinary(data, sizeInBytes))
    {
        ...
        ScaleState restoredState;
        restoredState.rootNote = juce::jlimit(0, 11, static_cast<int>(tree.getProperty("rootNote", 0)));
        restoredState.scaleName = tree.getProperty("scaleName", "Major").toString();
        restoredState.sourceLabel = tree.getProperty("sourceLabel", "Manual preset").toString();
        restoredState.detailText = tree.getProperty("detailText", "Choose a root and scale, or analyse a file.").toString();
        restoredState.derivedFromAnalysis = static_cast<bool>(tree.getProperty("derivedFromAnalysis", false));

        const auto maskBits = tree.getProperty("maskBits", "").toString();
        for (int i = 0; i < juce::jmin(12, maskBits.length()); ++i)
            restoredState.enabledPitchClasses[static_cast<size_t>(i)] = maskBits[i] == '1';

        setScaleState(restoredState);
    }
}

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new AggregaScaleAudioProcessor();
}
